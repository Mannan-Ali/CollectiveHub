...
const getAllVideos = asynHandler(async (req, res) => {
  ...
  /*steps:
    ...
    4.pagination
    */
  //here we are checking if the values entered for page if entered it shoud be greater than 0 and not characters.
  //  If such things are done we pass default values like 1 and 10
  const validatedPage =
...
const getVideoById = asynHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, video, "Follwing vidoe data was fetched"));
});
...
const deleteVideo = asynHandler(async (req, res) => {
  ...
  if (deleteOldVideo.result !== "ok") {
    throw new ApiError(500, "Error while deleting old video from cloudinary");
  }
  ...
  if (!deleteVideoOject) {
    throw new ApiError(500, "Error while deleting old video file on Atlas");
  }
  return res
    .status(200)
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        video,
        `Publishing was successfully set to ${video.isPublished}`
      )
    );
});
