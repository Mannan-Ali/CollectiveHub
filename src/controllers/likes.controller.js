  ...
  try {
    ...
    if (deleteVideoLike) {
      ...
      return res
        .status(200)
        .json(
          new ApiResponse(201, newVideoLike, "Successfully liked the video")
        );
    }
  } catch (error) {
  ...
  try {
    ...
    if (deleteCommentLike) {
      ...
      return res
        ...
        .json(
          new ApiResponse(
            200,
            deleteCommentLike,
            "Successfully unliked the comment !!"
          )
        );
    } else {
      ...
      return res
        .status(200)
        .json(
          new ApiResponse(201, newCommentLike, "Successfully liked the comment")
        );
    }
  } catch (error) {
  ...
  try {
    ...
    if (deleteTweetLike) {
      ...
      return res
        .status(200)
        .json(
          new ApiResponse(201, newTweetLike, "Successfully liked the Tweet")
        );
    }
  } catch (error) {
...
const getLikedVideos = asynHandler(async (req, res) => {
  ...
  try {
    ...
    throw new ApiError(
      500,
      error?.message,
      "Error while returning allLikeVideos."
    );
  }
});
